...
class HParams(object):
  ...
  def _init_from_proto(self, hparam_def):
    ...
    """
    for name, value in six.iteritems(hparam_def.hparam):
      ...
      if kind.endswith('_value'):
        ...
        if kind.startswith('int64'):
          ...
          self.add_hparam(name, tf.compat.as_str(getattr(value, kind)))
        else:
        ...
        if kind.startswith('int64'):
          ...
          # with both Python2 and Python3. UTF-8 encoding is assumed.
          self.add_hparam(
              name, [tf.compat.as_str(v) for v in getattr(value, kind).value])
        else:
