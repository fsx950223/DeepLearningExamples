...
class ResnetModel(object):
    ...
    def __init__(
        ...
        self.model_hparams = hparam.HParams(
            n_classes=n_classes,
        ...
        self.batch_norm_hparams = hparam.HParams(
            decay=0.9,
            epsilon=1e-5,
            scale=True,
            center=True,
        ...
        self.conv2d_hparams = hparam.HParams(
        ...
        self.dense_hparams = hparam.HParams(
            kernel_initializer=tf.variance_scaling_initializer(
                scale=2.0, distribution='truncated_normal', mode=weight_init
            ),
            bias_initializer=tf.constant_initializer(0.0)
        ...
        if mode != tf.estimator.ModeKeys.PREDICT:
            ...
            if mode == tf.estimator.ModeKeys.TRAIN and params['quantize']:
                ...
                if params['symmetric']:
                    ...
                    tf_quantize.create_training_graph(
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            ...
            if mode == tf.estimator.ModeKeys.TRAIN:
                ...
                if self.model_hparams.use_dali:
                    ...
                    train_ops = tf.group(
                        backprop_op, update_ops, name='train_ops'
                    )
